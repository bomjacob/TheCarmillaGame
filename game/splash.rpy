init python:
    if not persistent.disclaimer_seen:
        persistent.disclaimer_seen = "no" #For some reason we can't use True and False
...
entirety of Season 1 before playing this game.{/size}""", text_align=0.5, xmaximum=740)
    image ctc:
        Text("""{size=10}Press any key to continue{/size}""", text_align=0.5, xmaximum=740)

    show disclaimer at top with dissolve
    if persistent.disclaimer_seen == "no":
    ...
    hide ctc
    $ persistent.disclaimer_seen = "yes"
